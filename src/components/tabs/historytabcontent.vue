  ...
  export default {
    ...
    data() {
      return {
        ...
        active: 0,
        routers:[
          {name:'警情分布',toName:'historySpread'},
          {name:'高发区域',toName:'historyHighArea'},
          {name:'高发道路',toName:'historyHighRoad'},
          {name:'高发路口路段',toName:'historyHighCrossing'},
          {name:'天气分析',toName:'weather'},
          {name:'交通方式',toName:'vehicle'},
        ],
        metaName:this.$route.meta.name,
      }
    },
    mounted(){
      this.drawsdLine();
    ...
    methods: {
      ...
      onTabClick(item) {
        // this.active = i;
        this.$router.push(`/historyWarning/${this.metaName}/${item.toName}`);
      },
